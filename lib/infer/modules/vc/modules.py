        ...
        if split_audio:
            ...
            if resultm == "Finish":
                ...
                merge_timestamps_file = os.path.join(
                    ...
                    f"{os.path.basename(input_audio_path1).split('.')[0]}_timestamps.txt",
                )
                ...
                merged_audio_path = os.path.join(
                    os.path.dirname(new_dir_path),
                    "audio-outputs",
                    f"{os.path.basename(input_audio_path1).split('.')[0]}_merged.wav",
                )
                index_info = (
                    "Index:\n%s." % file_index
        ...
        try:
            ...
            while True:
                current_output_path = os.path.join(
                    opt_root, opt_filename.format(output_count)
                )
            ...
            try:
                if format1 in ["wav", "flac"]:
                    sf.write(
                        "%s.%s" % (current_output_path, format1),
                        audio_opt,
                        self.tgt_sr,
                    )
